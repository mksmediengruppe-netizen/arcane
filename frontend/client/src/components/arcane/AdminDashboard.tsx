// ARCANE Admin Dashboard — "Warm Intelligence" design
...
import { PermissionDeniedScreen } from "@/components/arcane/PermissionDeniedScreen";
import {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Обзор системы ARCANE</p>
      </div>
...
function ChatsSection({ searchQuery, setSearchQuery }: { searchQuery: string; setSearchQuery: (v: string) => void }) {
  ...
  const loadChats = (p: number, q: string) => {
    ...
    fetch(`/api/admin/chats?${params}`, { headers: { Authorization: `Bearer ${localStorage.getItem('arcane_session_token')}` } })
      .then(r => r.json())
...
function CostsSection() {
  const breakdown = [
    { user: "Алексей Петров",  tasks: 28, cost: 18.42, budget: 50.00, model: "Arcane Standard" },
    { user: "Мария Иванова",   tasks: 19, cost: 11.20, budget: 30.00, model: "Arcane Fast" },
    { user: "Дмитрий Сидоров", tasks: 12, cost: 9.80,  budget: 25.00, model: "Arcane Pro" },
    { user: "Анна Козлова",    tasks: 7,  cost: 3.15,  budget: 20.00, model: "Arcane Fast" },
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Глобальные настройки системы ARCANE</p>
      </div>
      ...
      <div className="bg-white border border-[#E8E6E1] rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              <option value="fast">Arcane Fast</option>
              <option value="standard">Arcane Standard</option>
              <option value="pro">Arcane Pro</option>
            </select>
          </div>
        </div>
      </div>
