...
import {
  ...
  Code2, FileJson, Layers, Eye, CheckCircle2, Circle, Loader2,
  Terminal, Search as SearchIcon, MousePointer, Edit3
} from "lucide-react";
...
// ─── Current Tool Indicator (Manus-style shimmer) ────────────────────────────

const TOOL_DISPLAY: Record<string, { label: string; icon: React.ReactNode }> = {
  shell_exec:       { label: "Выполняет команду", icon: <Terminal size={12} /> },
  file_write:       { label: "Записывает файл", icon: <FileText size={12} /> },
  file_read:        { label: "Читает файл", icon: <Eye size={12} /> },
  file_edit:        { label: "Редактирует файл", icon: <Edit3 size={12} /> },
  browser_navigate: { label: "Открывает страницу", icon: <Globe size={12} /> },
  browser_click:    { label: "Кликает в браузере", icon: <MousePointer size={12} /> },
  browser_input:    { label: "Вводит текст", icon: <Globe size={12} /> },
  browser_scroll:   { label: "Прокручивает страницу", icon: <Globe size={12} /> },
  ssh_exec:         { label: "SSH команда", icon: <Terminal size={12} /> },
  search_web:       { label: "Ищет в интернете", icon: <SearchIcon size={12} /> },
  image_generate:   { label: "Генерирует изображение", icon: <ImageIcon size={12} /> },
  design_judge:     { label: "Оценивает дизайн", icon: <Eye size={12} /> },
  message:          { label: "Отправляет сообщение", icon: <Bot size={12} /> },
};

function CurrentToolIndicator({ tool }: { tool: string }) {
  const display = TOOL_DISPLAY[tool] || { label: tool || "Работает...", icon: <Loader2 size={12} className="animate-spin" /> };
  return (
    <motion.div
      initial={{ opacity: 0, y: 4 }}
      animate={{ opacity: 1, y: 0 }}
      className="mt-2 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30 text-blue-700 dark:text-blue-300 text-xs font-medium overflow-hidden relative"
    >
      <div className="absolute inset-0 shimmer-effect" />
      <Loader2 size={11} className="animate-spin shrink-0 relative z-10" />
      <span className="opacity-60 relative z-10">{display.icon}</span>
      <span className="relative z-10">{display.label}</span>
    </motion.div>
  );
}

// ─── Plan Card (Manus-style task progress) ───────────────────────────────────
...
function PlanCard({ plan }: { plan: string[] }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="mt-2 bg-gray-50 dark:bg-[#1e2130] border border-gray-200 dark:border-[#2a2d3a] rounded-xl px-3 py-2.5"
    >
      <div className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">План задачи</div>
...
const FILE_TYPE_ICONS: Record<string, React.ReactNode> = {
  json: <FileJson size={14} className="text-amber-500" />,
  md: <FileText size={14} className="text-blue-500" />,
  html: <Globe size={14} className="text-green-500" />,
  css: <FileCode size={14} className="text-purple-500" />,
  js: <FileCode size={14} className="text-yellow-500" />,
  ts: <FileCode size={14} className="text-blue-600" />,
  py: <FileCode size={14} className="text-green-600" />,
  pdf: <FileText size={14} className="text-red-500" />,
  zip: <FileText size={14} className="text-gray-500" />,
  png: <ImageIcon size={14} className="text-pink-500" />,
  jpg: <ImageIcon size={14} className="text-pink-500" />,
  svg: <ImageIcon size={14} className="text-orange-500" />,
};

function getFileIcon(name: string) {
  const ext = name.split(".").pop()?.toLowerCase() || "";
  return FILE_TYPE_ICONS[ext] ?? <FileText size={14} className="text-gray-400" />;
}
...
async function authenticatedDownload(url: string, filename: string) {
  try {
    const token = localStorage.getItem("arcane_session_token");
    const resp = await fetch(url, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    });
    if (!resp.ok) {
      toast.error(`Ошибка загрузки: ${resp.status}`);
      return;
    }
...
function AttachmentCards({ attachments }: { attachments: AttachmentFile[] }) {
  return (
    <div className="mt-2 space-y-1.5">
      {attachments.map((file, i) => (
        ...
        >
          <div
            className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-[#E8E6E1] bg-[#F8F7F5] hover:bg-white transition-colors cursor-pointer group"
            onClick={() => authenticatedDownload(file.download_url, file.name)}
          >
            <div className="w-7 h-7 rounded-lg bg-white border border-[#E8E6E1] flex items-center justify-center shrink-0">
            ...
            <div className="flex items-center gap-1">
              <button
                className="p-1 rounded hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors opacity-0 group-hover:opacity-100"
                onClick={e => { e.stopPropagation(); authenticatedDownload(file.download_url, file.name); }}
                title="Скачать"
              >
                <Download size={12} />
              </button>
...
export function ChatMessage({ message, activeStep, onStepClick, isLast, isCompleted, onEdit, onArtifactOpen, planCompleted, planActiveIndex }: ChatMessageProps) {
  ...
  const hasContent = !!(message.content || message.currentTool || (message.steps && message.steps.length > 0) || (message.plan && message.plan.length > 0) || message.artifact || (message.viewerArtifacts && message.viewerArtifacts.length > 0));
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Current tool indicator (shimmer) — show whenever agent is using a tool */}
        {message.currentTool && (
        ...
        {/* PHASE-6: File attachments from message(type="result", attachments=[...]) */}
