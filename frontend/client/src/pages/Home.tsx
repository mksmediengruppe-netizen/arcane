...
import { Sidebar } from "@/components/arcane/Sidebar";
import { ChatHeader, type ModelKey, type ModelOptions } from "@/components/arcane/ChatHeader";
import { ChatMessage } from "@/components/arcane/ChatMessage";
import { Composer } from "@/components/arcane/Composer";
import { RightPanel } from "@/components/arcane/RightPanel";
...
import { AgentInterruptDialog, useAgentInterrupt } from "@/components/arcane/AgentInterruptDialog";
import { SkeletonChat } from "@/components/arcane/SkeletonChat";
import { motion, AnimatePresence } from "framer-motion";
import { Bot, BookOpen, Calendar } from "lucide-react";
import { PlaybooksPanel } from "@/components/arcane/PlaybooksPanel";
import { ScheduledTasksPanel } from "@/components/arcane/ScheduledTasksPanel";
import { useScheduleAPI } from "@/hooks/useScheduleAPI";
import { toast } from "sonner";
import { useTheme } from "@/contexts/ThemeContext";
import { type AppNotification } from "@/components/arcane/NotificationCenter";
import { UserSettingsPanel } from "@/components/arcane/UserSettingsPanel";
...
export default function Home() {
  ...
  const [modelOptions, setModelOptions] = useState<ModelOptions>({ premiumImages: false, designCheck: false, premiumReview: false });
  ...
  const handleSend = useCallback((text: string) => {
    ...
    if (!currentChatId) {
      ...
      chatsAPI.createChat("Новая задача").then((newChat) => {
        if (newChat) {
          chatsAPI.sendMessage(newChat.id, text, selectedModel, modelOptions);
        }
      });
      return;
    }
    chatsAPI.sendMessage(currentChatId, text, selectedModel, modelOptions);
  }, [budgetExhausted, chatsAPI, selectedModel, modelOptions]);
